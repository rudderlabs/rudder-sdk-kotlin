...
determine_base_branch() {
    ...
    echo "Determining base branch for release comparison..."
    ...
    if [ "$is_release_branch" = "true" ]; then
        ...
        if remote_branch_exists "develop"; then
            base_branch="develop"
            echo "Release branch detected - using develop as base"
        elif remote_branch_exists "main"; then
            base_branch="main"
            echo "⚠️  develop branch not found, falling back to main"
        else
...
validate_base_branch() {
    ...
    echo "✅ Base branch origin/$base_branch exists and is valid"
}
...
main() {
    ...
    echo ""
    echo "✅ Base branch determination completed successfully"
