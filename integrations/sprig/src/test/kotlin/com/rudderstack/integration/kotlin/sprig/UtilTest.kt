...
class UtilTest {
    ...
    @Nested
    inner class SetSprigAttributes {
        ...
        fun `given attributes with key starting with exclamation, when setSprigAttributes is called, then attribute is skipped`() {
            ...
            verify { mockLogger.warn(match { it.contains("cannot start with '!'") }) }
        }

        @Test
        fun `given attributes with key of 256 chars, when setSprigAttributes is called, then key is trimmed and attribute is set`() {
            val longKey = "a".repeat(256)
            val trimmedKey = "a".repeat(255)
            val attributes = buildJsonObject { put(longKey, "value") }

            setSprigAttributes(Sprig, attributes, mockLogger)

            verify { Sprig.setVisitorAttribute(trimmedKey, "value") }
            verify { mockLogger.warn(match { it.contains("Trimming") }) }
        }
